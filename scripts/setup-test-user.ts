...
async function setupTestUser() {
  ...
  const email = await question('Email (test@gymbro.com): ')
  const password = await question('Password: ')

  const { data: authData, error: authError } = await supabase.auth.signInWithPassword({
    email: email.trim() || 'test@gymbro.com',
  ...
  if (authData.user.id !== userId) {
    console.error(`⚠️ Warning: El user_id autenticado (${authData.user.id}) no coincide con el esperado (${userId})`)
    console.log('   Usando el user_id autenticado...')
  }
  ...
  console.log('\nDatos de prueba listos para test@gymbro.com:')
  console.log('- 1 rutina activa con 2 sesiones')
  console.log('- 3 ejercicios en Push A')
  console.log('- Martes completado con 9 sets de tracking')
  console.log('- Fondos con 2/3 series (caso de uso parcial)')
  console.log('\nAhora puedes:')
  console.log('1. Loguearte como test@gymbro.com')
  console.log('2. Verificar que el calendario muestre martes completado')
  console.log('3. Ir a Métricas → Progreso y ver los datos de Press Banca, Press Militar y Fondos')
  console.log('4. Verificar que la adherencia se calcule correctamente (1 sesión completada)')
}
